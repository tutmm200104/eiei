...
// 与 iframe 内 calculateWaterReward / getTodaySpecialDate 保持一致
const COUPLE_GARDEN_NORMAL_REWARD = 5.20;
const COUPLE_GARDEN_ANNIV_REWARD = 1314.00;
const COUPLE_GARDEN_DEFAULT_SPECIAL_DATES = [
  { date: '01-01', name: '元旦', coins: 100.00 },
  { date: '02-14', name: '情人节', coins: 520.00 },
  { date: '03-08', name: '妇女节', coins: 38.00 },
  { date: '03-14', name: '白色情人节', coins: 314.00 },
  { date: '05-01', name: '劳动节', coins: 51.00 },
  { date: '05-20', name: '520', coins: 520.00 },
  { date: '05-21', name: '521', coins: 521.00 },
  { date: '06-01', name: '儿童节', coins: 61.00 },
  { date: '07-07', name: '七夕', coins: 77.77 },
  { date: '10-01', name: '国庆节', coins: 101.00 },
  { date: '11-11', name: '光棍节', coins: 111.10 },
  { date: '12-24', name: '平安夜', coins: 124.00 },
  { date: '12-25', name: '圣诞节', coins: 125.00 },
...
function calculateCoupleSpaceGardenWaterReward(charId, atTime) {
  ...
  for (let i = 0; i < COUPLE_GARDEN_DEFAULT_SPECIAL_DATES.length; i++) {
    if (COUPLE_GARDEN_DEFAULT_SPECIAL_DATES[i].date === mmdd) {
      return { amount: COUPLE_GARDEN_DEFAULT_SPECIAL_DATES[i].coins, special: COUPLE_GARDEN_DEFAULT_SPECIAL_DATES[i] };
    }
  }
  try {
    const gardenData = JSON.parse(localStorage.getItem('coupleGarden_' + charId) || '{}');
    const specialDates = gardenData.specialDates || [];
    for (let j = 0; j < specialDates.length; j++) {
      if (specialDates[j].date === mmdd) {
        return { amount: Number(specialDates[j].coins) || COUPLE_GARDEN_NORMAL_REWARD, special: specialDates[j] };
      }
    }
  } catch (e) {}
  try {
    const annivs = JSON.parse(localStorage.getItem('coupleAnniv_' + charId) || '[]');
    for (let k = 0; k < annivs.length; k++) {
      const ad = annivs[k].date;
      if (ad) {
        const parts = ad.split('-');
        if (parts.length >= 3 && (parts[1] + '-' + parts[2]) === mmdd) {
          return { amount: COUPLE_GARDEN_ANNIV_REWARD, special: { date: mmdd, name: annivs[k].title, coins: COUPLE_GARDEN_ANNIV_REWARD } };
        }
      }
    }
  } catch (e) {}
...
async function triggerAutoGardenWater(charId, isTimer = false) {
  ...
  try {
    ...
    const reward = calculateCoupleSpaceGardenWaterReward(charId);
    const newWater = {
      ...
      specialDate: reward.special ? { name: reward.special.name, coins: reward.special.coins } : null,
      hearts: { char: true },
      comments: []
    };
    const iframe = document.getElementById('couple-space-iframe');
    ...
    if (isIframeOpenForThisChar && iframe.contentWindow) {
      ...
      try {
        const gd = JSON.parse(localStorage.getItem('coupleGarden_' + charId) || '{}');
        if (!gd.waterLogs) gd.waterLogs = [];
        gd.waterLogs.push(newWater);
        gd.totalCoins = (gd.totalCoins || 0) + reward.amount;
        localStorage.setItem('coupleGarden_' + charId, JSON.stringify(gd));
        await handleCoupleSpaceGardenWaterReward({
          charId,
          author: 'char',
          amount: reward.amount,
          description: reward.special ? ('情侣树浇水-' + reward.special.name) : '情侣树自动浇水奖励'
        });
